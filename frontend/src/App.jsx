...
let CameraIcon = L.divIcon({
  ...
  html: '<div style="background:#2563eb; color:white; border-radius:50%; width:24px; height:24px; display:flex; align-items:center; justify-content:center; border:2px solid white; box-shadow:0 2px 6px rgba(0,0,0,0.3); font-size:11px;">≡ƒô╖</div>',
  iconSize: [24, 24],
  iconAnchor: [12, 12]
});
    ...
    <div className="app-shell">
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-section">
          ...
          >
            <Camera size={18} />
            <span className="nav-label">Camera Traps {showCameraStations && "Γ£ô"}</span>
          </button>
          ...
          >
            <Layers size={18} />
            <span className="nav-label">All Territories {showMultiTerritories && "Γ£ô"}</span>
          </button>
        </div>
                ...
                <div className="map-container">
                  ...
                  >
                    ...
                    {overlaps.map((ov, idx) => (
                      ...
                      >
                        <Popup>
                          <strong style={{ color: '#c94a45' }}>ΓÜá∩╕Å Territory Overlap Zone</strong><br/>
                    ...
                    {uploadStatus && (
                      ...
                      >
                        {uploadStatus.status === "success" ? (
                          <>
                            <strong>≡ƒÉ» Tiger Match Detected</strong><br />
                          ...
                          <>
                            <strong>≡ƒ¢í∩╕Å Triage: Image Quarantined (Blank)</strong><br />
                            {uploadStatus.message}
                          </>
                        ) : (
                          <>
                            <strong>Γ¥î Processing Error</strong><br />
                            {uploadStatus.message}
                          </>
                        )}
                      </div>
                    ...
                    {bulkResult && (
                      <div className={`result-box ${bulkResult.status === 'success' ? 'result-success' : 'result-error'}`}>
                        {bulkResult.status === 'success' ? (
                          <>
                            <strong>≡ƒôé Triage Report Complete</strong><br/>
                          ...
                          <>
                            <strong>Γ¥î Triage Error</strong><br/>
                            {bulkResult.message}
                          </>
                        )}
                      </div>
